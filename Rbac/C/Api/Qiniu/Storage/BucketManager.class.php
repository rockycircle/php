<?php
namespace Rbac\C\Api\Qiniu\Storage;
use Rbac\C\Api\Qiniu\Pub;
use Rbac\C\Api\Qiniu\Auth;
use Rbac\C\Api\Qiniu\Config;
use Rbac\C\Api\Qiniu\Http\Client;
use Rbac\C\Api\Qiniu\Http\Error;
 ...
 */
final class BucketManager extends Pub
{
    private $auth;
    ...
    public function listFiles($bucket, $prefix = null, $marker = null, $limit = 1000, $delimiter = null)
    {
        ...
        $this->setWithoutEmpty($query, 'prefix', $prefix);
        $this->setWithoutEmpty($query, 'marker', $marker);
        $this->setWithoutEmpty($query, 'limit', $limit);
        $this->setWithoutEmpty($query, 'delimiter', $delimiter);
    ...
    {
        $path = '/stat/' . $this->entry($bucket, $key);
    ...
    {
        $from = $this->entry($from_bucket, $from_key);
        $to = $this->entry($to_bucket, $to_key);
        $path = '/copy/' . $from . '/' . $to;
        list(, $error) = $this->rsPost($path);
        return $error;
    }
    ...
    private static function oneKeyBatch($operation, $bucket, $keys)
    {
        $data = array();
        foreach ($keys as $key) {
            array_push($data, $operation . '/' . $this->entry($bucket, $key));
        }
        return $data;
    }
    ...
    private static function twoKeyBatch($operation, $source_bucket, $key_pairs, $target_bucket)
    {
        ...
        foreach ($key_pairs as $from_key => $to_key) {
            $from = $this->entry($source_bucket, $from_key);
            $to = $this->entry($target_bucket, $to_key);
