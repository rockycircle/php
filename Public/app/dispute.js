$(function(){
    ...
    $('#manager-dispute').datagrid({

        url:Think.APP+"Dispute/index",//用Extend控制器的index将数据取出来

        fit: true, //自适应
        striped: true,
        fitColumns: true,
        toolbar: '#dispute-product-toolbar',
        border: false, //取消边框
        ...
        onClickRow: function (rowIndex, rowData) {//单击事件
            $('#manager-dispute').datagrid("clearChecked"); //取消所有行的选中
            $('#manager-dispute').datagrid("selectRow", rowIndex);  //选中当前行
            $('#manager-dispute-user').datagrid({ url: Think.APP + "Order/ordersProduct&orders_id=" + rowData.orders_id });  //重新加载数据
        }
    })

    $('#dispute-product').datagrid({
        fit: true, //自适应
        striped: true,
        fitColumns: true,
        toolbar: '#dispute-product-toolbar',
        ...
        onClickRow: function (rowIndex, rowData) {//单击事件
            $('#dispute-product').datagrid("clearChecked"); //取消所有行的选中
            $('#dispute-product').datagrid("selectRow", rowIndex);  //选中当前行
    ...
    dispute_toolbar={
        deleteOrder:function(){
            var url=Think.APP+"Dispute/del&cmd=orders";
            Xwindow.baseDelete("#manager-dispute", url, "orders_id");

        },
        deleteProduct:function(){
            var url=Think.APP+"Dispute/del&cmd=orders_product";
            Xwindow.baseDelete("#dispute-product", url, "orders_product_id");
        },
        searchOrder:function (value, name) {

            $('#manager-dispute').datagrid("load", {
                "orders_id": value,
            });
        },
        openSearchOrders:function(){
            ...
            Xwindow.baseDialogInit("#manager-dispute", "#dispute-search-form", "高级搜索",'search');
        },
        //获取二维码URL
        selectQrCode:function () {
            var selectData = $("#dispute-product").datagrid("getSelections");
            ...
            if (selectData == "") {
                ...
                //     temp[i]=selectData[i]['product_qr_code'];
                //     console.log(temp);
                // }
